...
class FrmExercicioAlterarBD_01_15022024(tk.Frame):
...
class FrmTrvwPreferencias(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.btnExcluirBD = tk.Button(self.FrmBotoes, width=self.btnWidth, background='grey', text='Excluir', command=self.ExcluirBD)
        self.btnExcluirBD.pack(side='top', padx=self.btnPadx, pady=self.btnPady)

        self.btnAlterarBD = tk.Button(self.FrmBotoes, width=self.btnWidth, background='grey', text='Alterar', command=self.AlterarBD)
...
class FrmDelPreferencia(tk.Tk):
    def __init__(self, objFrmTrvwPreferencias, *args, **kwargs):
        ...
        # self.lblDescricao = tk.Label(self, text=descricaoItemDaLinhaSelecionada, anchor='w')
        # self.lblDescricao.pack(side='top', fill='x')
...
class FrmAltPreferencia(tk.Tk):
    ...
    def AltPreferencia(self):
        ...
        f'Confirma alteração de Preferencia {self.lblDescricao.get()} pela Preferência {self.txtDescricao.get()}?') == True:
            try:
                connectionString = (
                    r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};'
                    r'DBQ=C:\CURSO PROGRAMAR\Python\Preferencias_1_05072023.accdb;'
